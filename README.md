...
- `.env` 파일 접근이 Conductor 내부에서 차단됨. 로컬 터미널에서는 정상 접근 가능
...
| 스킬/익스텐션 추가 불가 | 익스텐션 API (`registerCommand`, `registerTool`, `registerShortcut`, `ui.custom` TUI 등)로 자유롭게 확장. 현재 24개 익스텐션, 16개 스킬 직접 구현 |
...
24개. 도구를 등록하지 않는 익스텐션(spinner, session-title 등)은 토큰 영향 0.

### 인프라

| 이름 | 설명 |
|------|------|
| **claude-code-ui** | Read/Write/Edit/Bash 렌더링 + 커스텀 푸터 + 도구 출력 기본 접힘 |
| **subagent** | 병렬 서브에이전트 — hang 감지 5min, auto-retry 3x, `ask_master` 에스컬레이션, `/subagent-replay` TUI |
...
| **worktree** | Git worktree 관리 — 포켓몬 1세대 한글 이름, `--hotfix`/`--hotfeature`, 멀티 레포 |
...
| 이름 | 설명 |
|------|------|
| **idle-screensaver** | 5분 비활성 → 포켓몬 스프라이트 + 마지막 맥락 + TODO 표시 |
...
16개. 글로벌 워크플로 스킬만 pilee에 포함. 프로젝트 고유 스킬(create-pr, self-review)은 해당 레포에 위치.
...
| 스킬 | 역할 |
|------|------|
| **stress-interview** | 3 병렬 에이전트 코드 리뷰 (원본 6축 → 3축, 토큰 50% 절감) |
...
stress-interview에서 사용하는 3개 에이전트. 모두 `claude-opus-4-6`.

| 에이전트 | 관점 |
|---------|------|
| **verifier** | 구현이 요구사항을 충족하는가 |
| **reviewer** | 코드 품질, 패턴, 유지보수성 |
| **challenger** | 반론, 엣지 케이스, 놓친 시나리오 |

`scripts/sync-agents.mjs` (postinstall)로 `~/.pi/agent/agents/`에 자동 동기화.
...
| 원인 | 영향 | 대응 |
|------|------|------|
| cc-system-prompt + pi 시스템 프롬프트 이중 적재 | +2K/턴 | **cc-system-prompt 제거** — tft-guidelines로 대체 |
...
├── extensions/     # 24개
├── skills/         # 16개
├── agents/         # 3개 (verifier, reviewer, challenger)
