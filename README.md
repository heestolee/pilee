...
/wt bootstrap [status]       → profile 기반 의존성 조건부 background worker
...
- profile이 bootstrap 대상으로 지정한 worktree에서 구현이 시작되면 누락된 의존성만 `/wt` background worker가 조건부 bootstrap한다. 수동 확인은 `/wt bootstrap status`.
