...
- profile이 bootstrap 대상으로 지정한 worktree에서 구현이 시작되면 `/wt`가 AI subagent orchestrator를 띄워 deterministic executor를 실행·진단하게 한다. 수동 확인은 `/wt bootstrap status`, deterministic fallback은 `/wt bootstrap --executor`.
...
| skill | `skills/git-workflow-and-versioning` | [변경 통합은 작은 단위와 검증을 요구한다](./docs/knowledge/change-integration-discipline.md)<br>[변경된 줄은 요청으로 추적 가능해야 한다](./docs/knowledge/request-traceability-surgical-changes.md) |
...
41개. 도구를 등록하지 않는 익스텐션(spinner, session-title 등)은 토큰 영향 0.
...
23개. 글로벌 워크플로 스킬만 pilee에 포함. 프로젝트 고유 스킬(create-pr, self-review)은 해당 레포/private overlay에 위치.
