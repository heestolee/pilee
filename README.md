...
```text
/frame-v2 --draft <주제>  조사된 최초 학습노트를 먼저 만들기
/frame-v2 <주제>          현재 Frame 질문 규율로 함께 만들기
            ↓
TFT Studio 시각화 → Study Hard에서 소화·revision → HTML/Notion → frame.json → 작업 시작
```

`/frame-v2`는 기존 `/frame`과 `/study-hard`를 바꾸지 않는 독립 pilot이다. Frame의 구조화·ERD·flow 표현과 Study Hard의 질문·답변·문서 다듬기를 순서대로 연결한다.
...
| skill | `skills/frame-v2` | [Frame v2는 이해를 학습노트로 만든 뒤 작업 계약으로 승격한다](./docs/knowledge/frame-v2-learning-note-pilot.md)<br>[학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md) |
