...
### Frame v2 작업·학습 coordinator pilot
...
Frame / Study Hard / 구현은 이후 순차 또는 병렬로 연결
```

`/frame-v2`는 기존 `/frame`과 `/study-hard`를 바꾸지 않는 독립 pilot이다. 명령 직후 시작 방향을 선택하고, Frame 기획·Study Hard 학습·구현을 같은 work unit에서 양방향으로 연결한다. 학습 완료를 구현 hard gate로 만들지 않으며, Frame이 있으면 Study Hard 최상단에 전체 기획을 기본 접힘으로 보여준다. Standalone `/study-hard` run도 나중에 같은 runId로 Frame에 연결할 수 있다.
...
- `/wt switch`는 metadata-only 목록을 먼저 열고, 현재 화면에 보이는 행만 bounded concurrency로 Git 상태를 지연 갱신합니다.
...
| extension | `extensions/auto-commit` | [Auto-commit은 명시 계획만 실행한다](./docs/knowledge/auto-commit-explicit-plan-gate.md)<br>[Repo status polling은 index.lock을 만들지 않도록 조율한다](./docs/knowledge/repo-status-index-lock-coordination.md)<br>[Slice 완료는 commit 후보를 만든다](./docs/knowledge/slice-auto-commit-rhythm.md) |
...
| extension | `extensions/convention-lens` | [Convention Lens는 작업 종료 시 관련 판단만 투영한다](./docs/knowledge/convention-lens-event-driven-projection.md) |
...
| extension | `extensions/frame-v2` | [Frame v2는 작업·학습 lane을 양방향으로 연결한다](./docs/knowledge/frame-v2-learning-note-pilot.md)<br>[학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md) |
...
| extension | `extensions/learning-companion` | [학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md)<br>[Study Hard 엔진은 public이고 개인 publisher만 private다](./docs/knowledge/study-hard-public-engine-private-publisher.md) |
| extension | `extensions/mcp-bridge` | [검토 산출물은 다시 열 수 있어야 한다](./docs/knowledge/artifact-archive-reopenability.md)<br>[Deterministic fallback은 workflow를 보존한다](./docs/knowledge/deterministic-fallbacks-preserve-workflow.md)<br>[MCP 결과는 구조화 출력부터 digest-first로 다룬다](./docs/knowledge/mcp-digest-first-artifacts.md)<br>[MCP runtime은 장시간 세션에서 self-healing해야 한다](./docs/knowledge/mcp-runtime-self-healing.md)<br>[MCP stderr는 TUI 출력이 아니다](./docs/knowledge/mcp-stderr-isolation.md)<br>[터미널 연동은 host adapter로 다룬다](./docs/knowledge/terminal-host-integration.md)<br>[도구 출력은 대화 흐름을 침범하지 않는다](./docs/knowledge/tool-output-noise-management.md) |
...
| extension | `extensions/pr-review` | [Convention Lens는 작업 종료 시 관련 판단만 투영한다](./docs/knowledge/convention-lens-event-driven-projection.md)<br>[Meta Review는 전체 diff 설명 뒤에 독립 판단과 precedent를 붙인다](./docs/knowledge/human-pr-review-precedent-harness.md)<br>[학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md)<br>[질문 UI와 실행 owner는 분리한다](./docs/knowledge/question-ui-execution-owner-routing.md)<br>[Study Hard 엔진은 public이고 개인 publisher만 private다](./docs/knowledge/study-hard-public-engine-private-publisher.md)<br>[작업공간 동작과 panel activation은 별도 계약이다](./docs/knowledge/workspace-action-panel-activation-contract.md) |
...
| extension | `extensions/questions` | [질문 UI와 실행 owner는 분리한다](./docs/knowledge/question-ui-execution-owner-routing.md) |
| extension | `extensions/queued-messages` | [병렬 workflow 분석은 단일 writer queue로 합류한다](./docs/knowledge/parallel-workflow-analysis-single-writer.md)<br>[Queued command는 실행 보장이 아니다](./docs/knowledge/queued-command-prefill-boundary.md) |
...
| extension | `extensions/ship-commands` | [CI-Ship은 PR 후 검증 실패 대응 단계다](./docs/knowledge/ci-ship-failure-response-boundary.md)<br>[병렬 workflow 분석은 단일 writer queue로 합류한다](./docs/knowledge/parallel-workflow-analysis-single-writer.md)<br>[Ship과 PR-Ship은 서로 다른 통합 단계다](./docs/knowledge/ship-pr-ship-review-boundary.md)<br>[Subagent는 slash command가 아니라 skill prompt를 위임받는다](./docs/knowledge/subagent-skill-delegation.md) |
...
| extension | `extensions/study-hard` | [Frame v2는 작업·학습 lane을 양방향으로 연결한다](./docs/knowledge/frame-v2-learning-note-pilot.md)<br>[Meta Review는 전체 diff 설명 뒤에 독립 판단과 precedent를 붙인다](./docs/knowledge/human-pr-review-precedent-harness.md)<br>[학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md)<br>[질문 UI와 실행 owner는 분리한다](./docs/knowledge/question-ui-execution-owner-routing.md)<br>[Study Hard 엔진은 public이고 개인 publisher만 private다](./docs/knowledge/study-hard-public-engine-private-publisher.md)<br>[Study Hard worker는 유연하게 생성하고 extension coordinator가 엄격하게 적용한다](./docs/knowledge/study-hard-worker-flexible-generation-strict-apply.md)<br>[TFT visual은 구조 변화를 학습 가능한 그림으로 보여준다](./docs/knowledge/tft-visual-structure-renderer.md) |
| extension | `extensions/subagent` | [Worker는 readiness ownership을 가진다](./docs/knowledge/ai-worker-readiness-orchestrator.md)<br>[Convention Lens는 작업 종료 시 관련 판단만 투영한다](./docs/knowledge/convention-lens-event-driven-projection.md)<br>[최종 검증은 메인 세션을 막지 않고 병렬화한다](./docs/knowledge/final-verification-parallelization.md)<br>[병렬 workflow 분석은 단일 writer queue로 합류한다](./docs/knowledge/parallel-workflow-analysis-single-writer.md)<br>[질문 UI와 실행 owner는 분리한다](./docs/knowledge/question-ui-execution-owner-routing.md)<br>[Queued command는 실행 보장이 아니다](./docs/knowledge/queued-command-prefill-boundary.md)<br>[Self-healing은 actionable item만 수정한다](./docs/knowledge/self-healing-actionable-loop.md)<br>[Stress Interview는 다축 검토다](./docs/knowledge/stress-interview-multi-axis-review.md)<br>[Study Hard worker는 유연하게 생성하고 extension coordinator가 엄격하게 적용한다](./docs/knowledge/study-hard-worker-flexible-generation-strict-apply.md)<br>[Hybrid subagent 모델 운용 정책](./docs/knowledge/subagent-model-policy.md)<br>[Subagent 위임은 구체 프롬프트를 요구한다](./docs/knowledge/subagent-prompt-specificity.md)<br>[Subagent는 slash command가 아니라 skill prompt를 위임받는다](./docs/knowledge/subagent-skill-delegation.md)<br>[Ultra는 Max reasoning과 proactive delegation을 결합한 로컬 mode다](./docs/knowledge/ultra-proactive-delegation-mode.md) |
...
| extension | `extensions/utils` | [Convention Lens는 작업 종료 시 관련 판단만 투영한다](./docs/knowledge/convention-lens-event-driven-projection.md)<br>[TFT Studio는 TFT 단계를 작업 단위 UI로 묶는다](./docs/knowledge/frame-studio-interactive-decision-ui.md)<br>[MCP stderr는 TUI 출력이 아니다](./docs/knowledge/mcp-stderr-isolation.md)<br>[Read/Edit tool override는 필요한 증거만 펼친다](./docs/knowledge/read-edit-tool-output-override.md)<br>[Repo status polling은 index.lock을 만들지 않도록 조율한다](./docs/knowledge/repo-status-index-lock-coordination.md)<br>[Session export는 원본을 보존하는 adapter를 거친다](./docs/knowledge/session-export-source-preservation.md)<br>[Study Hard 엔진은 public이고 개인 publisher만 private다](./docs/knowledge/study-hard-public-engine-private-publisher.md)<br>[터미널 연동은 host adapter로 다룬다](./docs/knowledge/terminal-host-integration.md)<br>[Utils surface는 사용자 계약을 만들지 않는다](./docs/knowledge/utility-surface-stays-invisible.md)<br>[Working Context Card는 큰 맥락을 현재 slice로 압축한다](./docs/knowledge/work-context-card-task-board.md) |
...
| extension | `extensions/workflow-guard` | [외부 Issue와 PR은 기여 규칙 확인과 최종 승인 뒤에 게시한다](./docs/knowledge/external-issue-preview-gate.md)<br>[장시간 세션은 phase와 stop-line으로 제어한다](./docs/knowledge/long-running-session-control.md)<br>[Slice 완료는 commit 후보를 만든다](./docs/knowledge/slice-auto-commit-rhythm.md)<br>[Ultra는 Max reasoning과 proactive delegation을 결합한 로컬 mode다](./docs/knowledge/ultra-proactive-delegation-mode.md)<br>[Working Context Card는 큰 맥락을 현재 slice로 압축한다](./docs/knowledge/work-context-card-task-board.md)<br>[반복 워크플로 실패는 guard/flow로 고정한다](./docs/knowledge/workflow-guard-enforced-flow.md)<br>[작업공간 동작과 panel activation은 별도 계약이다](./docs/knowledge/workspace-action-panel-activation-contract.md) |
...
| skill | `skills/ci-ship` | [CI-Ship은 PR 후 검증 실패 대응 단계다](./docs/knowledge/ci-ship-failure-response-boundary.md)<br>[병렬 workflow 분석은 단일 writer queue로 합류한다](./docs/knowledge/parallel-workflow-analysis-single-writer.md)<br>[Runtime fan-out은 healthcheck 뒤의 실행 계약이다](./docs/knowledge/runtime-fanout-diagnosis.md)<br>[Subagent는 slash command가 아니라 skill prompt를 위임받는다](./docs/knowledge/subagent-skill-delegation.md) |
...
| skill | `skills/meta-review` | [Meta Review는 전체 diff 설명 뒤에 독립 판단과 precedent를 붙인다](./docs/knowledge/human-pr-review-precedent-harness.md) |
...
| skill | `skills/pr-ship` | [학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md)<br>[병렬 workflow 분석은 단일 writer queue로 합류한다](./docs/knowledge/parallel-workflow-analysis-single-writer.md)<br>[Ship과 PR-Ship은 서로 다른 통합 단계다](./docs/knowledge/ship-pr-ship-review-boundary.md)<br>[Subagent는 slash command가 아니라 skill prompt를 위임받는다](./docs/knowledge/subagent-skill-delegation.md) |
| skill | `skills/runtime-fanout-triage` | [Runtime fan-out은 healthcheck 뒤의 실행 계약이다](./docs/knowledge/runtime-fanout-diagnosis.md) |
| skill | `skills/self-healing` | [Worker는 readiness ownership을 가진다](./docs/knowledge/ai-worker-readiness-orchestrator.md)<br>[병렬 workflow 분석은 단일 writer queue로 합류한다](./docs/knowledge/parallel-workflow-analysis-single-writer.md)<br>[Self-healing은 actionable item만 수정한다](./docs/knowledge/self-healing-actionable-loop.md)<br>[Hybrid subagent 모델 운용 정책](./docs/knowledge/subagent-model-policy.md)<br>[Subagent 위임은 구체 프롬프트를 요구한다](./docs/knowledge/subagent-prompt-specificity.md)<br>[테스트는 책임 경계에 맞춰 다듬는다](./docs/knowledge/test-boundary-refactor.md) |
| skill | `skills/ship` | [외부 Issue와 PR은 기여 규칙 확인과 최종 승인 뒤에 게시한다](./docs/knowledge/external-issue-preview-gate.md)<br>[최종 검증은 메인 세션을 막지 않고 병렬화한다](./docs/knowledge/final-verification-parallelization.md)<br>[학습노트 companion은 독립 canonical을 sidecar로 연결한다](./docs/knowledge/learning-note-companion-artifact.md)<br>[Ship과 PR-Ship은 서로 다른 통합 단계다](./docs/knowledge/ship-pr-ship-review-boundary.md)<br>[Subagent는 slash command가 아니라 skill prompt를 위임받는다](./docs/knowledge/subagent-skill-delegation.md) |
...
| skill | `skills/test-boundary-refactor` | [Convention Lens는 작업 종료 시 관련 판단만 투영한다](./docs/knowledge/convention-lens-event-driven-projection.md)<br>[테스트는 책임 경계에 맞춰 다듬는다](./docs/knowledge/test-boundary-refactor.md) |
...
| skill | `skills/to-production` | [To-production은 현재 worktree의 production 기반 branch 전환이다](./docs/knowledge/to-production-source-preserving-hotfix.md) |
...
63개. 도구를 등록하지 않는 익스텐션(spinner, session-title 등)은 토큰 영향 0.
...
| **frame-v2** | `/frame-v2` — 최초 학습노트·Frame 시각화·Study Hard refinement·작업 시작을 잇는 독립 pilot |
| **study-hard** | `/study-hard` — concept map·flow·Q&A·revision·HTML/Notion export를 제공하는 범용 학습 board |
...
| **convention-lens** | `agent_settled`에서 diff 관련 convention만 projection해 shadow·review·safe repair를 수행 |
...
| **to-production** | 현재 worktree를 최신 production 기반 새 branch로 전환하고 기존 commit을 cherry-pick |
...
| **questions** | TODO: inventory 설명 필요 |
...
| **frame-v2** | 초안 먼저/질문형 학습노트를 만들고 Study Hard로 소화한 뒤 표준 frame과 작업으로 전환 |
...
| **meta-review** | 현재 작업·외부 PR의 모든 diff를 설명하고 실제 리뷰 판단·revision을 Study Hard 코드 리뷰 탭에서 관리 |
...
| **to-production** | 현재 worktree를 production 기반 hotfix branch로 전환하고 기존 commit을 cherry-pick |
...
11개. `scripts/sync-agents.mjs` (postinstall)로 `~/.pi/agent/agents/`에 자동 동기화.

### 서브에이전트 (`>>` 심볼)

| 에이전트 | 심볼 | 모델 | 역할 |
|---------|------|------|------|
| **worker** | `>>` (기본) | gpt-6-astra (max, fallback: Sol) | 범용 구현/수정 |
| **finder** | `>>/` | gpt-6-astra (low, fallback: Luna) | 파일/코드 탐색 (read/grep/find only) |
| **searcher** | `>>?` | gpt-6-astra (high, fallback: Terra → Sol) | 웹 리서치/문서 조사 |
| **planner** | `>>#` | gpt-6-astra (max, fallback: Sol) | 구현 계획 설계 |
| **challenger** | `>>!` | claude-opus-5 (max, fallback: Astra → Sol) | 계획/결정의 숨은 가정 공격 |
| **browser** | `>>@` | gpt-6-astra (high, fallback: Sol) | playwright E2E, UI 확인 |
| **bootstrapper** | 내부 `/wt` | gpt-6-astra (low, fallback: Luna) | 의존성 readiness orchestrator |
| **meta-review-question-worker** | 내부 Meta Review | gpt-6-astra (max, fallback: Sol → Terra → Spark) | 고정 source 질문·patch artifact |
| **study-hard-worker** | 내부 Study Hard | gpt-6-astra (max, fallback: Sol → Terra → Spark) | 학습 노트 제안 artifact |

### 리뷰 (`/stress-interview`)

| 에이전트 | 관점 |
|---------|------|
| **verifier** | 요구사항 충족 증거 판정 (claude-opus-5, max, fallback: Astra → Sol) |
| **reviewer** | 코드 품질·정확성·보안 검토 (gpt-6-astra, max, fallback: Sol) |
