...
- *Markdown 변환 지원 MCP*: `contentFormat: "markdown"` 같은 명시 옵션이 있을 때만 markdown을 보낸다. Jira Cloud에서는 이 옵션이 markdown → ADF 변환 성공 경로가 될 수 있다.
- *Authenticated browser session REST 경로*: MCP가 부족해도 로그인된 브라우저 세션 쿠키를 사용해 REST v3에 ADF object를 직접 보낼 수 있다. 구체 구현은 private/project overlay에 둔다.
...
Jira Cloud rich formatting이 필요하면 ADF를 사용한다. 가능한 경로는 대략 세 가지다.

1. ADF object를 직접 받는 REST/MCP.
2. `contentFormat: "markdown"`처럼 markdown을 ADF로 변환한다고 명시한 MCP.
3. 인증된 브라우저 세션 쿠키를 이용한 REST v3 ADF PUT. 이 방법은 조직/환경별 민감 구현이므로 private overlay 지침을 따른다.

어느 경로든 전송 후 다시 읽어서 ADF node가 실제로 저장됐는지 확인한다.
...
```text
포맷 경로: ADF 직접 전송 / Markdown→ADF 변환 / Browser-session REST ADF / Wiki markup / Plain fallback
사용 도구: MCP/API/overlay helper 이름
