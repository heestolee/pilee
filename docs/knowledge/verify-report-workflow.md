...
reviewed_commit: 1493846dacb54c7a9352836abecd8bbb44546a43
related:
...
기본 검증 구조는 evidence-first입니다. main agent가 coverage 계획과 증거 수집을 통제하고, 여러 검증 축이 있으면 evidence bundle을 만든 뒤 case별 verifier subagent를 병렬 실행해 판정 의견을 받습니다. Subagent는 주어진 evidence를 읽고 `PASS`/`FAIL`/`UNVERIFIED`를 반환할 뿐이며, 최종 report 상태와 추가 증거 수집/사용자 질문은 main agent가 처리합니다.
...
## Case Verifier Fan-out Rule

`/verify-report`는 별도 `--workers` 모드를 노출하지 않습니다. 기본값은 “main이 증거를 수집하고, case verifier subagent가 증거를 교차검증하며, main이 최종 판정한다”입니다. 사용자가 `--no-workers`를 지정하거나 단일·자명한 항목이면 main이 직접 판정합니다.

Fan-out은 증거 수집을 subagent에게 떠넘기는 기능이 아닙니다. 브라우저 로그인, 캡처, 네트워크/콘솔 로그 수집, side-effect 가능 작업은 main이 책임집니다. Subagent는 이미 저장된 evidence bundle만 읽어야 하며, 부족하면 `UNVERIFIED`와 `main_action_required`로 올립니다. 이 escalation은 실패가 아니라 main이 추가 캡처, 기준 재해석, 사용자 질문, Coverage Gap 중 하나를 선택하게 하는 안전장치입니다.

Subagent verdict는 untrusted input입니다. Main은 evidence path 존재 여부와 criteria closure를 확인한 뒤에만 `verify_report_live` item을 `pass`/`fail`/`unverified`로 업데이트합니다.
...
- 여러 검증 축은 기본적으로 evidence bundle 기반 case verifier fan-out을 사용하되, `--no-workers`는 main 직접 판정 escape hatch입니다.
