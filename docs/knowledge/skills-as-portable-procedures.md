...
반복 가능한 절차라고 해서 항상 사용자-facing skill command로 노출해야 하는 것은 아닙니다. 자연어 트리거와 `/skill:<name>` 직접 호출이 유용한 절차는 skill로 둡니다. 반대로 사용자가 하나의 slash command만 보아야 하고 command shim이 인자 처리·prerequisite inline·merge gate 같은 실행 맥락을 붙이는 workflow라면, 실행 계약을 extension 내부 `WORKFLOW.md`처럼 skill discovery 밖에 둘 수 있습니다. 이 경우 canonical 절차는 유지하되 `/skill:<name>`과 `/command`가 동시에 보여 사용자에게 두 진입점처럼 보이는 혼란을 피합니다.
