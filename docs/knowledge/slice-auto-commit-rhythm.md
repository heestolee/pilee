...
다만 slice 하나가 곧 commit 하나라는 뜻은 아닙니다. slice가 여러 domain/layer/surface를 포함해 primary file이 3개 이상으로 커지면, `auto_commit`의 logical atom gate가 더 작은 reviewable commit으로 쪼개도록 막아야 합니다.
...
4. 한 commit entry에 primary path가 3개 이상이면 slice commit으로 만족하지 말고 logical atom 단위로 plan을 쪼갭니다. test/generated/schema/package metadata은 companion으로만 붙입니다.
