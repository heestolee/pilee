...
6. 커밋을 보류하면 `work_context action=checkpoint`에 이유를 남깁니다. migration 실행, UI capture, 최종 verify-report 미완료는 ship readiness caveat일 수 있지만, nearest validation을 통과한 코드 slice의 commit blocker로 단독 사용하지 않습니다.
