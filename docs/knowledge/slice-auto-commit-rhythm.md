...
4. 적절하면 `auto_commit action=apply planPath=<planPath>`로 커밋합니다.
5. 커밋을 보류하면 `work_context action=checkpoint`에 이유를 남깁니다.
...
- `auto_commit`은 여전히 explicit JSON plan만 실행합니다.
