...
Pi 대화에 slash command 문자열을 queue했다고 해서 그 command가 실제로 실행된다고 가정하면 안 됩니다. 세션 전환이나 worktree 생성처럼 실행 경계가 중요한 작업은 “메시지 주입”이나 입력창 prefill이 아니라, 실제 session switch API가 있는 command context에서만 수행합니다.
