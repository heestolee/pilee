...
WebView에 렌더된 HTML+SVG 혼합 visual을 PNG로 저장할 때 `foreignObject → canvas.toDataURL()`이 모든 host에서 동작한다고 가정하지 않습니다. WKWebView는 이 canvas를 taint해 export를 막을 수 있으므로 macOS Glimpse adapter는 Node→host `snapshot` command를 `WKWebView.takeSnapshot`으로 처리합니다. 가로·세로로 viewport보다 큰 visual은 snapshot rect만 키우지 말고 숨은 WebView 자체를 전체 visual 크기로 먼저 resize해야 빈 영역 없이 캡처됩니다. native snapshot이 없는 host에서는 browser capture fallback을 사용하며, 어느 경우에도 PNG가 canonical visual spec을 대체하지 않습니다.
