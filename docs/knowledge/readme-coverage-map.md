...
README coverage map은 knowledge 상태의 출력물입니다. root `README.md`와 `README.en.md`의 generated block이나 `docs/knowledge/README.md`가 바뀌었다는 이유만으로 doctrine을 다시 stale하게 만들지 않습니다. coverage map은 stale을 발견하는 입력이 아니라, 현재 연결 상태를 사람이 읽게 해주는 projection입니다.
