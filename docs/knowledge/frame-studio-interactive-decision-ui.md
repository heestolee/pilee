...
Reopen은 읽기 전용 artifact 보기에서 끝나면 안 됩니다. 사용자가 기획을 이어가려는 경우 `/tft open <transcriptPath>` 또는 같은 identity의 `/tft open`으로 live TFT Studio를 재개할 수 있어야 합니다. `done`/`aborted` transcript도 재진입 시 status를 다시 `running`으로 전환해 후속 질문·수정이 같은 timeline에 이어져야 하며, 새 transcript를 만들지 않습니다.
