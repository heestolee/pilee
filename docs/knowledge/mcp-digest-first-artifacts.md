...
- 모든 MCP tool result의 기본 `text`는 한 줄 카드로 반환합니다. 전체 readable digest는 `details.fullDigest`에 보존하고, 기존 tool output 펼침 단축키 `Ctrl+O`로 펼칩니다. 이는 원문을 버리는 것이 아니라 UI 기본 표시만 줄이는 규칙입니다.
