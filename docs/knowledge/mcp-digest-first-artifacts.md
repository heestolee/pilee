...
- 모든 MCP tool result의 UI 기본 표시는 한 줄 카드입니다. 모델-visible `content.text`에는 카드와 `responseId`/`get_mcp_content` locator만 담고, 전체 readable digest는 `details.fullDigest`에 보존해 기존 tool output 펼침 단축키 `Ctrl+O`로 보여줍니다. 이는 원문을 버리는 것이 아니라 UI 기본 표시와 모델용 lazy-retrieval 핸들을 분리하는 규칙입니다.

이 기준은 UI collapse가 아니라 context 절감 규칙입니다. 원문을 이미 model context에 넣고 접는 것이 아니라, 처음 반환부터 locator만 싣고 필요할 때 원문을 가져옵니다.
