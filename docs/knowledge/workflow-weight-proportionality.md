...
- light 작업에서 PR/status 확인을 사용자가 명시하지 않았다면 push 성공이 종료 조건입니다. 사용자가 불만을 표한 뒤 말하는 “걍 커밋푸시해” 같은 탈출 문구에만 의존하지 말고, 그 전에 push가 성공한 시점에서 PR/branch/status 확인을 덧붙이지 않고 즉시 짧게 완료 보고합니다.
- tool result 이후 판단 예산도 작업 무게에 비례합니다. none/light investigation은 30초 안에 다음 좁은 tool call·중간 결론·scope-gate 질문·최종 보고 중 하나를 선택하고, standard는 60초, full은 120초를 기본 예산으로 둡니다. 조용히 수분 동안 tool/schema/context 탐색 여부를 고민하는 것은 무게 비례 원칙 위반입니다.
