...
승인된 web search review는 Verify Report나 Frame transcript와 섞이지 않게 artifact browser에서 별도 종류로 다시 열 수 있어야 합니다. 검색 review archive는 “그때 승인한 출처와 요약”을 보여주는 회고용 증거이지, 이후에도 출처 내용이 변하지 않는다는 보장은 아닙니다.

archive를 다시 열 때도 curator의 승인 경계를 유지합니다. `/show-report`에서 web-search review를 preview하면 사용자는 같은 artifact browser 흐름 안에서 목록으로 돌아가거나 원본 HTML을 브라우저로 열 수 있어야 합니다. reopen UX가 깨지면 승인한 출처와 요약의 감사 가능성도 약해집니다.
