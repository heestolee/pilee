...
reviewed_commit: 84cb2323104c076809462c0a0661ef5eb75d54de
related:
...
이 분류는 canonical history가 아니라 navigation layer입니다. 원본 대화, session export, Verify Report, TFT Studio transcript는 그대로 두고 `/show-report`에서만 사람이 찾기 쉬운 category/tag/summary/segment view를 제공합니다.
...
AI는 분류의 최종 결정자가 아니라 초안 작성자입니다. `/show-report`가 세션 대화를 읽어 AI segment suggestion을 만들 수 있지만, 사용자가 확인하고 저장하기 전까지는 sidecar에 쓰지 않습니다.
...
`/show-report`는 session card에 `분류` action을 제공하고, category/tag/summary/segments를 사용자가 직접 수정할 수 있게 합니다. 목록에는 category filter를 제공해 `pilee 개선`, `영상 분석`, `잡담/방향성`, `업무 검증`처럼 사용자가 만든 맥락으로 세션을 찾을 수 있어야 합니다.
...
- `/show-report` filter가 원본 artifact 접근성을 방해할 때
