...
세션이 worktree로 실제 전환됐다면, session header의 cwd와 전환 직후 기록되는 worktree path가 실행 경계의 source of truth입니다. 특정 tool runner나 외부 harness가 이전 `pwd`를 계속 보고하더라도, 에이전트는 같은 전환을 사용자에게 다시 요구하지 말고 표시된 worktree 절대경로 아래에서 read/edit/bash/frame/verify artifact를 수행해야 합니다. 반대로 현재 패널이 worktree session으로 전환되지 않았다면, 절대경로만 들고 구현을 계속하면 안 됩니다. `/frame`과 Frame Studio 같은 identity 계산도 `ctx.cwd`만 믿지 말고 현재 session file의 header cwd를 먼저 읽어 worktree-bound artifact로 승격해야 합니다.

planning frame에서 `fork해서 시작`을 선택한 경우에도 실행 경계는 “worktree가 생겼다”가 아니라 “현재 패널이 forked worktree session으로 전환됐다”가 기준입니다. 그래서 `frame_worktree_fork`는 저장된 `/frame` command context로 실제 `/wt fork` handler를 호출하고, 전환 성공 후 새 session에 구현 follow-up을 넣습니다. 전환할 command context가 없으면 worktree를 만들지 않습니다.
...
실행 경계 없이 cwd만 바꿔 작업하면 세션 맥락, 브랜치, 검증 산출물이 서로 섞입니다. 세션 전환은 성공했는데 tool cwd만 뒤처진 경우를 전환 실패로 오판하면 전환 명령을 반복시키고 planning frame을 worktree-bound frame으로 승격하지 못합니다. 반대로 세션 전환 자체가 불가능한 tool context에서 절대경로 작업으로 우회하면 사용자가 기대한 context fork가 아니므로 즉시 `BLOCKED`로 멈춰야 합니다. 특히 병렬 패널과 subagent가 늘어날수록 worktree boundary가 안전장치가 됩니다.
