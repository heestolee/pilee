...
worktree session이 활성화됐다면 session header의 cwd와 exact session file이 실행 경계의 source of truth입니다. Slash `/wt new`는 worktree/session 생성 후 현재 panel을 그 exact target으로 직접 전환하고 compact continuation을 전달합니다. Slash `/wt fork`와 Frame/TFT command-context fork는 공통으로 현재 panel을 직접 전환하거나 새 탭·오른쪽 panel의 durable READY를 확인한 뒤 full-lineage continuation을 전달합니다. Full session 로딩이 초기 대기 시간을 넘겨도 실행 실패 근거가 아니므로 새 panel을 닫지 않고 pending activation을 유지합니다. `worktree_create`·`worktree_fork` 일반 tool은 source-panel/new-panel READY 계약을 유지합니다. PR review는 head-pinned review worktree를 실행 경계로 유지하면서도 활성화 위치는 `현재 패널 | 새 탭` 중 사용자가 고릅니다. 현재 패널 선택은 source worktree의 HEAD를 직접 바꾸지 않고 exact review session으로 전환하는 동작입니다. `/wt switch`·`worktree_switch`는 current panel을 기존 worktree session으로 전환합니다. 어떤 경로든 session 전환이 실패했다면 절대경로만 들고 구현을 계속하지 않습니다. `/frame`과 Frame Studio 같은 identity 계산도 `ctx.cwd`만 믿지 말고 현재 session file의 header cwd를 먼저 읽어 worktree-bound artifact로 승격해야 합니다.

planning frame에서 `fork해서 시작`을 선택한 경우 실행 경계는 “worktree가 생겼다”가 아니라 “사용자가 고른 panel의 exact session이 활성화되고 첫 implementation slice continuation이 전달됐다”가 기준입니다. `frame_worktree_fork`는 저장된 `/frame` command context와 composed workflow의 full-lineage 검증을 사용하되, panel 선택은 slash `/wt fork`의 `현재 패널 | 새 탭 | 오른쪽 패널` 인터페이스를 재사용합니다. source work-unit의 Frame/task는 ticket 인자 유무와 무관하게 함께 승격합니다. command context, lineage, 선택한 panel activation 중 하나라도 성립하지 않으면 생성 전 또는 정리 후 BLOCKED로 멈춥니다.
