...
Deterministic action은 CLI가 동일 입력에 항상 같은 출력으로 처리할 수 있어야 합니다. 예: generated README block 재생성, link graph 검증, frontmatter 필수값 검사. AI action은 후보 해석, 문서 분리/병합, private 문맥 sanitization처럼 판단이 필요한 일입니다.
