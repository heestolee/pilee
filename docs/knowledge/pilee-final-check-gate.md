...
6. runtime E2E가 필요한 변경이면 `pi update`/reload 이후 실제 사용자-visible 경로 확인
7. 구멍 발견 시 수정 후 재검증
8. local pilee-history/Notion sync와 push 상태 확인
...
## Runtime E2E after update/reload

pilee extension, tool renderer, slash command, WebView/Glimpse UI처럼 현재 Pi runtime에 로드되어야만 의미가 있는 변경은 unit test와 TypeScript syntax check만으로 닫지 않습니다. `pi update`는 package clone과 dependency를 최신화하지만 이미 떠 있는 세션의 extension module을 자동 교체한다고 가정하면 안 됩니다. 현재 세션에서 새 코드를 실제로 쓰려면 `/reload` 또는 이를 감싼 `/pilee-update` 같은 command-context reload가 필요합니다.

따라서 final-check는 다음을 구분합니다.

- **기계 검증**: syntax, unit test, deterministic contract test가 변경 계약을 고정하는지 확인합니다.
- **runtime 적용**: `pi update` 후 현재 세션이 reload되었는지 확인합니다.
- **사용자-visible E2E**: 실제 tool/slash command/WebView/외부 MCP 호출이 카드, 버튼, follow-up, model-readable content 같은 사용자 경험 계약을 만족하는지 확인합니다.

외부 서비스 연동 claim은 describe/schema나 mock만으로 PASS 처리하지 않습니다. Slack/Notion/Jira처럼 실제 consumer path가 있는 경우 최소 안전 데이터로 실제 호출까지 닫고, 실패하면 수정 → update/reload → E2E를 반복합니다. 자동화가 위험하거나 권한/데이터가 없어 불가능한 경우에는 PASS가 아니라 GAP/blocked로 남깁니다.
