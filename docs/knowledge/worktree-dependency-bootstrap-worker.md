...
4. `worktree_create`, `worktree_fork`, PR review, Frame/TFT처럼 별도 target process를 여는 composed workflow와 slash `/wt fork`의 새 탭·오른쪽 선택은 exact cwd/session의 durable READY 뒤에 create-specific bootstrap을 판정합니다. Slash `/wt new`와 `/wt fork`의 현재 panel 선택은 current-panel switch 뒤 compact/full continuation의 첫 구현 turn에서 일반 `defaultDomains`와 prompt/path rule을 적용합니다. READY 전 source panel이나 activation 실패로 정리할 worktree에서는 bootstrap process를 시작하지 않습니다.
...
따라서 자동 bootstrap은 subagent session(`~/.pi/agent/sessions/subagents/...`)과 bootstrap orchestrator prompt 안에서는 비활성화합니다. 또한 `[dependency-bootstrap] ...`, `/wt bootstrap status`, “왜 부트스트랩이 떴나”, “모바일 작업이 없는데 mobile bootstrap이 왜 관여하나”처럼 bootstrap 상태·구조·노이즈를 묻는 진단 프롬프트에서는 자동 bootstrap을 다시 시작하지 않습니다. Dependency readiness의 ownership은 main session이 orchestration하고, subagent는 전달받은 executor만 실행합니다.
