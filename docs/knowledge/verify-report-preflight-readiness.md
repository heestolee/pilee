...
반복 validation 실패는 사용자가 `/preflight` 명령을 직접 실행하는 책임이 아닙니다. Agent는 bash validation 결과의 preflight baseline 주석을 읽고, 새 unrelated baseline이라고 판단하면 `preflight_baseline` tool로 기록합니다. 수동 slash command는 사람이 cache를 확인·정리하고 싶을 때만 사용합니다.
