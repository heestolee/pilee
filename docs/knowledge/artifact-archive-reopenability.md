...
세션 JSONL은 raw line을 그대로 펼치기보다 user/assistant/tool/error timeline으로 변환해 보여줍니다. thinking signature/encrypted payload처럼 사람이 읽을 수 없거나 과도하게 긴 값은 숨기고, 전체 원본 확인은 allowlisted `브라우저에서 열기`로 분리합니다.
