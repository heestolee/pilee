...
5. planning work-unit의 task board를 새 worktree의 `.pi/work-tasks.json`으로 함께 옮기고, `.pi/work-context.json`을 새 frame 기준으로 refresh합니다.
...
Frame Step 9에서 `fork해서 시작`을 고르면 사용자는 별도 전환 명령을 의식하지 않아야 합니다. 이 선택은 일반 `worktree_fork` LLM tool이나 사용자 slash `/wt fork`가 아니라 `/frame` command shim이 저장한 command context를 재사용하는 `frame_worktree_fork` bridge로 처리합니다. bridge는 composed workflow의 엄격한 lineage 검증을 유지하되, 활성화 위치는 slash `/wt fork`와 같은 `현재 패널 | 새 탭 | 오른쪽 패널` 인터페이스로 고릅니다. 현재 패널은 exact fork session으로 전환하고 새 위치는 READY 뒤 활성화하며, 어느 쪽이든 planning frame·task board 승격과 “첫 ready implementation slice부터 이어서 작업” follow-up이 완료 기준입니다. ticket 인자가 없어도 source work-unit task board의 단일 Frame 참조를 따라 승격해야 합니다. command context나 full lineage가 없으면 BLOCKED로 멈추고, 사용자가 고르지 않은 panel이나 절대경로 작업으로 우회하지 않습니다.
