...
`git add`, `git commit`, `git reset`, `git push`, `git stash`, `git pull`처럼 index나 refs를 바꾸는 구간에서는 repo status polling을 pause합니다. `/update-branch`처럼 dirty 변경 보존을 위해 stash/pull/apply를 연속 실행하는 command도 같은 pause scope 안에서 실행합니다.
