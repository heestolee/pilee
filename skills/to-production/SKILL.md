...
현재 worktree의 변경사항을 **source worktree에 손대지 않고** 최신 production 기반 새 branch/worktree로 옮기는 workflow다. 실제 자동화는 `/to-production` extension command가 담당하고, 이 skill은 언제 실행하고 어떻게 검증/복구할지의 판단 기준이다.
