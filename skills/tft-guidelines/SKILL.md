...
### 철칙 1: 분기점 질문 의무
구현 방법이 2개 이상 존재하면, 하나를 선택하기 전에 반드시 AskUserQuestion으로 사용자에게 선택을 요청한다. **"명백해 보여도" 묻는다** — 단, "명백한" 후보가 정말로 1개인지 한 번 더 검토하고, 그래도 2개 이상이면 묻는다.
