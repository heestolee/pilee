...
- **UI 캡처 번들은 공통 기본값**: 모든 정적 UI 화면 검증은 `contextual focused crop`을 primary evidence로 두고, 같은 route/action/viewport의 full viewport screenshot을 supporting evidence로 함께 둔다. crop은 너무 좁게 잘라 label·컨테이너·주변 맥락을 잃지 않게 하고, 검증 포인트를 알아볼 만큼 주변 UI를 포함한다. user-facing Web UI는 desktop과 mobile viewport를 둘 다 기본 검증하고, 단일 플랫폼/admin desktop-only처럼 해당하지 않으면 생략 사유를 detail/Coverage Gap에 남긴다.
- **Coverage 먼저, 캡처는 그 다음**: 리포트 시작 전에 요구사항으로 검증 축을 정의한다. 캡처가 있어도 해당 축을 닫지 못하면 PASS가 아니다.
- **Oracle surface 먼저 정의**: 각 claim이 참/거짓으로 판정되는 전체 표면을 먼저 적는다. 표면은 “첫 화면에 보이는 일부”가 아니라 사용자가 판단해야 하는 전체 subject/action/state range다. 예: 전체 리스트, 전체 carousel/page set, 모든 option state, before/after 동일 interaction path, role별 허용/차단 경계, 저장 후 downstream 표시까지. Evidence가 이 판정 표면 전체를 같은 조작 경로로 덮지 못하면 PASS가 아니라 Coverage Gap이다.
...
검증 축은 필요하지만 데이터/계정/preview/before 기준이 불명확하거나, 작은 hotfix에 full report가 과할 수 있으면 먼저 `/verify-report-preflight`를 사용한다. 단, preflight는 선택적 확인 단계다. 사용자가 preflight를 생략해도 `/verify-report`는 이 스킬의 coverage/fan-out 규칙을 자체 적용해야 한다.
...
| oracle surface | 그 oracle을 판정하려면 전체 어디까지 봐야 하는지. 첫 viewport/첫 row/대표 1건인지, 전체 list/page/carousel/option/role/state/downstream까지인지 명시 |
...
그 다음 각 항목마다 **oracle surface gate**를 먼저 통과시킨다. Claim이 참/거짓으로 판정되는 전체 표면이 무엇인지 적고, evidence가 그 전체 표면을 같은 subject/role/viewport/action path로 덮는지 확인한다. “첫 화면에 3개 보임”, “대표 row 하나만 보임”, “after만 캡처함”, “다른 subject로 비교함”은 claim 전체를 닫지 못하면 supporting evidence일 뿐 PASS 근거가 아니다.

Oracle surface 예시:

| Claim 유형 | oracle surface |
|-----------|----------------|
| 리스트/캐러셀/페이지네이션 중복·정렬·누락 | 전체 item range와 순서. before/after가 있으면 같은 interaction path로 처음부터 끝까지 비교 |
| filter/search/result count | 입력 조건, empty/data/overflow 상태, 전체 result boundary 또는 sampling 기준 |
| option/default/selection | no data, existing data, refresh/stale, 저장 후 재조회 상태 |
| create/update action | 입력 → 저장 → reload/read → downstream user-facing 표시까지 |
| permission/role | 허용 role success path + 차단 role denial path + 잘못된 role을 blocker로 오인하지 않았는지 |
| responsive/layout | mobile/boundary/desktop의 문제 표면 전체. 한 viewport crop만으로 responsive claim PASS 금지 |
| event/network | trigger path, non-trigger path, payload/count, 중복 발화 여부 |

그 다음 각 항목을 **검증 축(coverage axis)** 으로 쪼개고, 축마다 **화면 캡처로 검증할지, 하단 보조 근거로 검증할지** 분류한다. 반복 카드·썸네일·badge·tag 정책은 여기서 consumer surface fan-out matrix를 반드시 만든다. 각 surface는 실제 route/subject를 찾고, 못 찾으면 PASS 대신 `exclusion` 또는 Coverage Gap으로 남긴다.

필수 coverage 도출 규칙:

| 변경 감지 | 기본 검증 축 |
|----------|--------------|
| 정적 UI 화면 검증 | contextual focused crop primary + same-route full viewport supporting + desktop/mobile 적용 여부 |
...
| 반복 카드/썸네일/badge/tag/list item 변경 | consumer surface matrix + 각 surface의 actual route subject + focused crop primary + full viewport supporting + mobile 별도 viewport + exclusion/gap 사유 |
...
| 캡처/증거 유형 | 형태 |
|----------|------|
| UI 단일 상태 | contextual focused crop primary + full viewport supporting. user-facing Web이면 desktop/mobile viewport를 둘 다 기본 캡처하고, 예외는 사유를 남긴다. |
| UI 다단계 플로우 / 스무스함 / 이동 | 고화질 GIF 또는 짧은 영상 primary + 대표 final-state contextual crop + full viewport supporting. 기본 GIF 생성은 `make-motion-gif.mjs` helper를 사용하고, 원본 해상도 유지 + 12fps + palette 최적화를 기본값으로 한다. |
...
- oracle surface 전체가 같은 subject/role/viewport/action path로 덮였을 때만 `pass`.
- flow/motion claim은 GIF/짧은 영상 primary evidence가 있어야 `pass`로 닫는다. 대표 final-state PNG/crop은 supporting으로 함께 둔다.
- 전체 range/list/page/carousel/option/role/downstream 판정이 필요한 claim을 첫 viewport, 첫 row, 대표 1건, final-state 1장만으로 닫지 않는다.
