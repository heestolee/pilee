...
`/verify`는 먼저 claim inventory를 만든다. claim은 frame success criteria, 사용자 요청, diff가 암시하는 user-facing 동작, 그리고 선택된 risk lens에서 나온다.

| Claim | Source | Evidence needed | PASS 조건 |
|---|---|---|---|
| 사용자/시스템에 참이어야 하는 문장 | frame/diff/request/lens | test, build, runtime, screenshot, artifact, log | evidence가 claim을 직접 닫음 |

규칙:
- claim이 evidence와 연결되지 않으면 PASS가 아니다.
- 코드 독해만으로는 user-facing behavior를 PASS 처리하지 않는다.
- UI/TUI/렌더링/리포트는 실제 화면 또는 artifact 캡처가 필요하다.
- 도구 호출 성공은 사용자 성공이 아니다.
- evidence를 만들 수 없으면 `부분`/`미검증`/`GAP`으로 남긴다.
...
- frame.json **있음** → success_criteria, out_of_scope, boundaries, verify_plan, decisions, requirement_matrix, domain_work_map, policy_axis_scan, backend_layer_map, architecture_flow_map을 메모리로 로드.
...
- frame.json `requirement_matrix`/`domain_work_map` → source-grounded requirement coverage lens
- frame.json `backend_layer_map.triggered` → backend layer responsibility lens
- frame.json `architecture_flow_map.triggered` → architecture/data flow lens
...
| ID | Statement | Evidence type | Grade | 근거 |
|---|---|---|---|---|
| SC-1 | 주문 단위 부분 환불 | test + code | 달성(코드+테스트) | refund.service.ts + spec PASS |
| SC-2 | 정산 시스템 반영 | code + manual gap | 달성(코드만) | 코드 매핑 OK, 수동 검증 필요 |
| SC-3 | 관리자 UI 노출 | UI capture missing | 부분 | UI 추가됐으나 i18n/캡처 누락 |
...
|---|---|---|---|---|
| R1 | 원문 요구 | diff/test/capture가 닫는 계약 | 파일/명령/캡처 | PASS/GAP/BLOCKED/OUT_OF_SCOPE |

| Domain lane | 닫아야 하는 requirement | 구현/검증 상태 | GAP |
|---|---|---|---|
| FE Admin | R1,R3 | 구현+캡처 필요 | 캡처 미실행이면 GAP |
| BE | R2 | usecase/repo/test 확인 | 없음 |
| DB/Ops | R4 | migration/runbook/dev result | dev 미실행이면 BLOCKED |
| Verification | R1-R4 | verify-report handoff 후보 생성 | subject identity 미정이면 BLOCKED |

규칙:
- 모든 requirement ID는 `PASS`, `GAP`, `BLOCKED`, `OUT_OF_SCOPE` 중 하나여야 한다.
- Domain Work Map leaf task가 닫히지 않았는데 SC만 PASS면 전체 verify는 Ready가 아니다.
- Verify는 최종 PM-facing report를 만들지 않는다. 다만 `/verify-report`에 넘길 후보 item은 `reuse/revise/add/drop/blocked` 이유와 함께 기록한다.
...
### Step 2.7: Architecture/Data Flow 검증

frame.json에 `architecture_flow_map.triggered`가 있으면 실제 diff가 lane/node/edge/source-of-truth 계약과 일치하는지 확인한다. 이 단계는 “레이어 책임은 맞지만 데이터가 다른 edge로 새거나 기존 source-of-truth를 우회함”을 잡기 위한 lens다.

| Flow element | 확인 질문 | PASS 근거 |
|---|---|---|
| lane coverage | UI/API/Usecase/Domain/Repository/DB/Ops 중 frame이 표시한 lane이 실제 구현·검증에 반영됐는가? | diff/test/capture/log |
| node mapping | 각 node가 frame의 책임과 같은 파일/컴포넌트/API/table에 대응하는가? | code locator + schema |
| edge semantics | 조회/write/승인/rollback/notification edge가 구현에서 같은 방향과 조건을 갖는가? | usecase/repo/log/test |
| source-of-truth | source-of-truth table/API/cache가 frame과 다르지 않은가? | schema/query/code evidence |
| requirement linkage | 주요 node/edge가 requirement ID와 연결되어 있고 미연결 ID가 없는가? | requirement coverage 표 |

불일치가 있으면 `Architecture/Data Flow findings`로 보고하고, PASS/GAP/BLOCKED를 requirement ID에 연결한다. PM-facing 화면 증거가 필요한 항목은 `/verify-report` handoff 후보로 넘기되, 기술적으로 대체 가능한 path라면 그 이유를 함께 남긴다.
...
frame.verifications.push({
  ...
  decisionAlignment: [...],
  requirementCoverage: [...],
  domainWorkMapCoverage: [...],
  architectureFlowFindings: [...],
  verifyReportHandoff: [...],      // reuse/revise/add/drop/blocked 후보와 이유
  edgeCasesHandled: [...],
  edgeCasesDeferred: [...],
...
**Requirement coverage**: {R1/R2별 PASS/GAP/BLOCKED/OUT_OF_SCOPE}
**Domain Work Map coverage**: {FE/BE/DB/Ops/Verification lane별 닫힘 여부}
**Architecture/Data Flow findings**: {lane/node/edge/source-of-truth 일치 여부}
...
- 배포 판정
- SC grade 요약
- requirement/domain work map coverage 요약
- risk lens PASS/GAP/BLOCKED 요약
- verify-report handoff 후보와 reuse/revise/add/drop/blocked 이유
