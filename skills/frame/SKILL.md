...
```markdown
현재 이해: {요청을 한 문장으로 요약}
막힌 결정: {지금 풀어야 하는 가장 중요한 불확실성 하나}
추천 답안: {있으면 제시. 없으면 "없음"이라고 쓰지 말고 생략}
질문: {한 가지 질문}
```
...
- Step 6/8: 현재 markdown을 `frame_studio action=update tab=frame`으로 렌더링한다.
...
- 가장 비싼 결정이 있으면 `막힌 결정` 또는 질문 앞 본문에 한 줄로 표시한다.
