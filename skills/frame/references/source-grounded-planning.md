...
|---|---|---|---|---|---|
| R1 | Jira §FE(1) | 기존 “대표 미디어” 영역 제거, “미디어” 영역 배치 | 공급자 관리 form에서 label/section 교체 | UI capture: old area absent + new area visible | pending |
| R2 | Jira §FE(1) | 운영자 관리 미디어 편집 컴포넌트 재사용 | 저장 callback과 presentational UI를 분리해 공통 UI import | code diff + requester/admin parity capture | decision-needed |
| R3 | Jira §FE(2) | 언어 선택 드롭다운, 존재 언어만 운영자 관리 순서 | language option builder + selected language state | dropdown option capture + language switch GIF | pending |
```
...
### FE Admin
- [R1,R2,R3] 공급자 관리 미디어 영역 교체, 언어 드롭다운, 공통 미디어 UI 재사용
  - 구현: requester basic info form, shared media editor UI
  - 검증: requester UI capture + language switch GIF
- [R5] Ops Review 언어별 원본/요청 비교
  - 검증: review screen capture

### BE
- [R4] `BASIC_INFO / LOCALIZED_MEDIA` payload를 언어별 목표 상태로 확장
  - 검증: GraphQL schema/codegen + create request test
- [R6] 승인 시 localized media source 반영
  - 검증: approval integration/unit test

### DB / Ops
- [R7] pending legacy representative media 요청 dry-run/반려/log
  - 검증: dry-run count, execute gate, 사후 SELECT/log artifact

### Verification
- [R1,R2,R3] requester/admin parity capture
...
|---|---|---|---|---|
| Entry/API | R4 | `localizedImages` input/output shape | DTO, GraphQL schema | schema/codegen |
| Application Flow | R4,R6 | 즉시 저장 금지, 요청 생성/승인 흐름 | create/approve usecase | integration test |
| Domain Rule | R3,R4 | 언어별 목표 상태, 대표 1개, 순서 | helper/VO/validation | unit test |
| Data Access | R6 | localized media source 조회/replace/order | repository calls | repo/mock test |
...
| Consumer | R1,R5 | requester/Ops UI가 같은 source를 표시 | FE query/component | UI capture |
```
...
현재 이해:
- 기획 근거는 언어별 미디어 목표 상태를 수정 요청 payload로 저장하라고 합니다.
- 승인 시 실제 source는 localized media source입니다.
