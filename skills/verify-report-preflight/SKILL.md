...
- **Frame은 requirement source, preflight는 handoff adjudicator**: TFT Frame의 Requirement Matrix/Domain Work Map/verify focus를 입력으로 삼되 그대로 복사하거나 무시하지 않는다. 각 Frame 항목은 `reuse`/`revise`/`add`/`drop`/`blocked`로 재판정해 `/verify-report`의 목차 초안으로 넘긴다.
- **Primary action 먼저**: 검증 대상 기능의 핵심 동사가 create/update/read-display/permission/event 중 무엇인지 먼저 고정한다. 과거 교정이나 실패 회고는 그 동사를 덮어쓰는 literal 요구가 아니라 intent 보존 제약으로 해석한다.
...
| 판정 | 질문 | preflight 처리 |
|---|---|---|
| `reuse` | Frame claim/subject/oracle/evidence가 현재 검증에도 그대로 맞는가? | Requirement ID를 유지해 V/T 후보로 승격 |
| `revise` | intent는 맞지만 subject/action/evidence가 현재 현실과 다른가? | Requirement ID와 수정 이유를 남기고 equivalent path로 재작성 |
| `add` | 최신 사용자 지시, 구현 diff, 데이터/권한 확인에서 새 검증축이 나왔는가? | 새 V/T 후보로 추가하고 출처를 명시 |
| `drop` | 현재 scope 밖, 중복, 오래된 계획인가? | 제외하되 이유를 `차단/주의`에 남김 |
| `blocked` | 대체 경로도 없어 검증할 수 없는가? | Coverage Gap/blocked 후보로 남김 |

수집 직후 과거 교정/실패 회고가 있으면 먼저 분해한다.

| 구분 | 질문 | 예시 |
|---|---|---|
| Primary action | 이번 기능을 실제로 닫는 핵심 동사는 무엇인가? | create/update/read-display/permission/event |
| Correction literal | 과거 교정 문장을 그대로 실행하면 무엇인가? | 특정 기존 row만 수정해서 비교 |
| Correction intent | 그 교정이 막으려던 실패는 무엇인가? | 서로 다른 조건의 데이터를 섞지 않기 |
| Feasibility | literal이 현재 권한/데이터/side effect 조건에서 현실적인가? | writer 정책, user-facing 노출 여부, DB write 위험 |
| Equivalent path | 비현실적이면 같은 intent를 보존하는 core feature path가 있는가? | 생성 기능이면 생성 가능한 subject로 새 row를 만들고 downstream 표시 확인 |

literal이 비현실적인데 equivalent path가 있으면 blocked가 아니라 그 경로를 contract에 명시한다. blocked는 equivalent path도 없거나 사용자 승인 없는 위험 side effect가 필요한 경우에만 쓴다.

### 2. PM-facing report contract 작성

각 요구를 아래 계약으로 바꾼다. 모르는 값은 추측하지 말고 `unknown`/`blocked`로 둔다.

| V | Frame handoff | 근거 출처 | PM-readable 성공 기준 | primary action | actor/role | subject identity | 화면 oracle | primary capture | 상태 |
|---|---------------|-----------|----------------------|----------------|------------|------------------|-------------|-----------------|------|
| V1 | reuse R1 / revise R2 / add | Jira/Notion/Slack/PR/user/frame | 비개발자가 이해할 한 문장 | create/update/read-display/permission/event | admin/member/partner/anonymous | 같은 row/order/review/item/user를 보장하는 id | 화면에 보여야 하는 텍스트/상태/흐름 | focused crop/GIF/viewport | ready/unknown/blocked |

작성 규칙:

- **근거 출처**는 “diff에서 봄”만으로 끝내지 않는다. 외부 기획 근거가 없으면 `사용자 지시`, `PR test plan`, `frame success criteria` 중 하나로 명시한다.
- **Frame handoff**는 Frame 항목을 복사하는 열이 아니라 evidence adjudication 결과다. `reuse`면 추적성을 유지하고, `revise`면 intent/Requirement ID와 변경 이유를 남기며, `add/drop/blocked`는 출처와 사유를 분리한다.
- **PM-readable 성공 기준**은 “mutation 성공”이 아니라 “관리자가 X를 선택하면 사용자 화면에서 Y로 보인다”처럼 쓴다.
- **primary action**은 회귀/보조 플로우보다 먼저 둔다. 생성 기능이면 생성 happy path와 저장/표시 결과가 1순위이고, 기존 항목 수정은 regression 축이다.
...
| Frame Handoff | Frame plan을 복사/무시하지 않고 재판정했는가? | 각 Frame item에 reuse/revise/add/drop/blocked와 이유 명시 |
| Primary Action / Correction | 기능의 핵심 동사와 과거 교정 intent가 충돌하지 않는가? | primary action 확정, literal 교정의 현실성 확인, 필요 시 equivalent path 명시 |
...
| V | Frame handoff | 근거 출처 | PM-readable 성공 기준 | primary action | actor/role | subject identity | 화면 oracle | primary capture | 상태 |
|---|---------------|-----------|----------------------|----------------|------------|------------------|-------------|-----------------|------|
| V1 | reuse/revise/add | ... | ... | create/update/read-display | ... | ... | ... | focused crop/GIF | ready |
...
- **subject identity가 없는 state transition**: 같은 subject를 잡을 방법을 먼저 묻거나 찾는다. 다만 과거 교정 literal이 현재 권한/데이터상 비현실적이고 기능의 primary action이 다른 경로라면, 교정 intent를 보존하는 equivalent subject/action을 contract에 명시하고 진행할 수 있다.
- **데이터 생성이 위험함**: DB write/결제/알림 side effect가 있으면 preflight에서 멈춘다. 단, 검증 대상 자체가 create 기능이고 사용자가 승인했거나 안전한 dev/preview fixture이면 생성 경로가 primary evidence가 될 수 있다.
