...
검증 명령을 돌리기 전후에 반복 실패를 분리한다.

```bash
/preflight baseline list
/preflight                  # profile이 있으면 관련 체크 실행
/preflight baseline add-last "unrelated baseline: <요약>" --expires 14d
```

- `baseline`으로 표시된 실패는 최종 보고에서 “이번 변경의 actionable failure 아님”으로 분리한다.
