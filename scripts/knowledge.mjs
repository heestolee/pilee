...
const README_EXTENSION_GROUPS = [
	{ ko: "인프라", en: "Infrastructure", items: [
		...
		{ name: "frame-v2", ko: "`/frame-v2` — 최초 학습노트·Frame 시각화·Study Hard refinement·작업 시작을 잇는 독립 pilot", en: "`/frame-v2` pilot connecting an initial learning note, Frame visuals, Study Hard refinement, and implementation start" },
	...
	{ ko: "세션 관리", en: "Session management", items: [
		...
		{ name: "to-production", ko: "현재 worktree를 최신 production 기반 새 branch로 전환하고 기존 commit을 cherry-pick", en: "Switches the current worktree to a production-based branch and cherry-picks existing commits" },
	...
	{ ko: "UI / UX", en: "UI / UX", items: [
		...
		{ name: "diff-overlay", ko: "`/diff [--base <branch>]` TUI — 실제 PR base 자동 인식, 커밋 모드, 파일 트리, 구문 하이라이팅", en: "`/diff [--base <branch>]` TUI with automatic PR-base resolution, commit mode, file tree, and syntax highlight" },
	...
	{ ko: "도구", en: "Tools and data", items: [
		...
		{ name: "test-refine", ko: "`/test-refine` — 테스트 mock/fixture/assertion을 책임 경계 기준으로 다듬는 command shim", en: "`/test-refine` command shim for refining test mocks, fixtures, and assertions by responsibility boundary" },
...
const README_SKILL_GROUPS = [
	{ ko: "핵심 사이클", en: "Core cycle", items: [
		...
		{ name: "frame-v2", ko: "초안 먼저/질문형 학습노트를 만들고 Study Hard로 소화한 뒤 표준 frame과 작업으로 전환", en: "Builds a draft-first or guided learning note, refines it in Study Hard, then promotes it to a standard frame and implementation" },
	...
	{ ko: "워크플로", en: "Workflow", items: [
		...
		{ name: "test-boundary-refactor", ko: "기능 테스트·내부 로직·외부 의존성 mock 경계 기준으로 테스트를 실용적으로 정리", en: "Practical test cleanup by behavior, internal logic, and external dependency mock boundaries" },
		...
		{ name: "to-production", ko: "현재 worktree를 production 기반 hotfix branch로 전환하고 기존 commit을 cherry-pick", en: "In-place switch of the current worktree to a production-based hotfix branch with cherry-pick replay" },
