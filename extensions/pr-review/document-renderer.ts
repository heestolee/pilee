...
function staticRuntimeScript(): string {
	...
	function cleanupMermaidRender(renderId){var wrapper=document.getElementById('d'+renderId);if(wrapper&&wrapper.parentElement===document.body)wrapper.remove();}
	function cleanupStaleMermaidErrors(){document.querySelectorAll('body > div[id^="d"] > svg[aria-roledescription="error"]').forEach(function(svg){var wrapper=svg.parentElement;if(wrapper&&wrapper.parentElement===document.body&&wrapper.id==='d'+svg.id)wrapper.remove();});}
	async function renderDiagrams(){cleanupStaleMermaidErrors();var targets=Array.from(document.querySelectorAll('[data-mermaid-source]'));if(window.mermaid)window.mermaid.initialize(${JSON.stringify(META_REVIEW_MERMAID_CONFIG)});for(var index=0;index<targets.length;index++){var target=targets[index],source=decodeURIComponent(target.dataset.mermaidSource||''),renderId='meta-review-static-'+(++renderVersion);try{if(!window.mermaid)throw new Error('offline');var result=await window.mermaid.render(renderId,source);target.innerHTML=result.svg;}catch(error){fallback(target,source);}finally{cleanupMermaidRender(renderId);}}}
	function closeVisual(){var overlay=document.getElementById('reviewMeaningVisualOverlay');if(overlay)overlay.hidden=true;}
	document.querySelectorAll('[data-review-visual-expand]').forEach(function(button){button.addEventListener('click',async function(){var visual=button.closest('.reviewMeaningVisual'),sourceTarget=visual&&visual.querySelector('[data-mermaid-source]'),source=sourceTarget&&decodeURIComponent(sourceTarget.dataset.mermaidSource||''),overlay=document.getElementById('reviewMeaningVisualOverlay'),stage=document.getElementById('reviewMeaningVisualOverlayStage'),renderId='meta-review-static-overlay-'+(++renderVersion);if(!overlay||!stage||!source)return;document.getElementById('reviewMeaningVisualOverlayTitle').textContent=button.dataset.reviewVisualTitle||'차트 확대 보기';document.getElementById('reviewMeaningVisualOverlayHint').textContent='읽는 법 · '+(button.dataset.reviewVisualHint||'원본 SVG를 크게 확인합니다.');overlay.hidden=false;try{if(!window.mermaid)throw new Error('offline');var result=await window.mermaid.render(renderId,source);stage.innerHTML=result.svg;}catch(error){fallback(stage,source);}finally{cleanupMermaidRender(renderId);}});});
