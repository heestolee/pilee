...
import { dispatchPrReviewQuestionToWorker, retryPrReviewQuestionToWorker } from "./question-worker.ts";
import {
	loadPrReviewRun,
	saveHumanDecision,
...
interface StudioHandle {
	...
	onRetry?: (questionId: string) => PrReviewQuestion;
}
...
(() => {
  ...
  function questionHtml(question){const context=[question.cardId,question.filePath,(question.evidenceIds||[]).join(', ')].filter(Boolean).join(' · '),change=question.change,changeHtml=change?'<div class="chat-evidence"><b>'+esc(change.status==='applied'?'로컬 변경 적용 완료':'로컬 변경 적용 · 확인 필요')+'</b><div>'+esc((change.files||[]).join(', '))+'</div>'+(change.refreshedRunId?'<div>리뷰 '+esc(change.refreshMode||'auto')+' 갱신 · '+esc(change.refreshedRunId)+'</div>':'')+(change.refreshError?'<div>갱신 오류 · '+esc(change.refreshError)+'</div>':'')+'</div>':'',retry=question.status==='failed'||question.status==='stale'?'<button data-question-retry="'+esc(question.id)+'">Worker로 다시 시도</button>':'';let response='';if(question.status==='answered')response='<div class="bubble coach">'+answerHtml(question.answer)+questionEvidenceHtml(question)+(question.uncertainty?'<div class="chat-evidence"><b>아직 확인이 필요한 것</b><div>'+esc(question.uncertainty)+'</div></div>':'')+changeHtml+'</div>';else if(question.status==='failed'||question.status==='stale')response='<div class="bubble coach failed">'+esc(question.error||'질문 처리에 실패했습니다.')+retry+'</div>';else response='<div class="bubble coach pending">공통 worker가 실제 source를 조사하고 있습니다…</div>';return'<div class="bubble learner">'+esc(question.question)+(context?'<div style="opacity:.68;font-size:10px;margin-top:5px">'+esc(context)+'</div>':'')+'</div>'+response;}
...
export async function startPrReviewStudioServer(
	...
	options: { onQuestion?: (question: PrReviewQuestion) => void; onRetry?: (questionId: string) => PrReviewQuestion } = {},
	...
	if (existing && !existing.closed) {
		...
		if (options.onRetry) existing.onRetry = options.onRetry;
		return existing;
	}
	...
	const server = createServer(async (request, response) => {
		try {
			...
			if (request.method === "POST" && url.pathname === "/retry") {
				...
				if (!questionId || !handle.onRetry) throw new Error("PR review question retry is not connected");
				const question = handle.onRetry(questionId);
				sendJson(response, 202, { ok: true, question });
				return;
			}
...
export async function openPrReviewStudio(
	...
	const handle = await startPrReviewStudioServer(state, {
		...
		onRetry: (questionId) => retryPrReviewQuestionToWorker(pi, state, questionId, ctx.cwd),
	});
