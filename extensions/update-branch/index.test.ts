...
test("argument parser supports help merge and no-autostash only", () => {
	assert.deepEqual(parseUpdateBranchArgs(""), { help: false, merge: false, noAutostash: false });
	assert.deepEqual(parseUpdateBranchArgs("--merge"), { help: false, merge: true, noAutostash: false });
	assert.deepEqual(parseUpdateBranchArgs("--no-autostash"), { help: false, merge: false, noAutostash: true });
	assert.deepEqual(parseUpdateBranchArgs("help"), { help: true, merge: false, noAutostash: false });
	assert.deepEqual(parseUpdateBranchArgs("--help --merge"), { help: true, merge: true, noAutostash: false });
...
test("runUpdateBranch can still block dirty worktree with no-autostash", async () => {
	...
	const result = await runUpdateBranch(pi as any, repo, "--no-autostash");
...
test("command registers slash command and emits visible summary", async () => {
	...
	assert.deepEqual(command.getArgumentCompletions("--no"), [{ value: "--no-autostash", label: "--no-autostash" }]);
