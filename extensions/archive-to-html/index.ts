		...
		try {
			...
			if (req.method === "GET" && url.pathname === "/session-search") {
				try {
					...
					const candidates = conversationSessionCandidates(data);
					const search = await searchSessionCandidates(candidates, query, { maxResults: 60, maxMatchesPerSession: 3 });
					const total = candidates.length;
					const summary = search.terms.length
						? `${search.results.length}개 세션에서 발견 · ${search.scanned}/${total} 세션 스캔${search.truncated ? " · 결과 제한으로 중단" : ""}`
						: `검색어를 입력하세요. 후보 세션 ${total}개`;
...
function conversationSessionCandidates(data: ArtifactBrowserData): ConversationSessionCandidate[] {
	const candidates = new Map<string, ConversationSessionCandidate>();
	const add = (candidate: ConversationSessionCandidate) => {
		let key = candidate.path;
		try { key = fs.realpathSync(candidate.path); } catch {}
	...
	for (const conductor of data.conductors) {
		for (const filePath of conductor.sourceSessionPaths) {
...
function totalSessionCandidates(data: ArtifactBrowserData): number {
	return conversationSessionCandidates(data).length;
}

function renderSessionSearchPlaceholder(data: ArtifactBrowserData): string {
	const total = totalSessionCandidates(data);
	return `<section class="card"><div class="kicker">대화 내용 검색</div><h2>대화 내용 검색</h2><p class="muted">상단 카드 검색과 별개로, Pi/Conductor session JSONL의 user/assistant 대화 본문을 직접 검색합니다.</p><div class="meta"><span class="badge">후보 세션 ${total}</span><span class="badge">matching snippet 표시</span><span class="badge">결과에서 세션 전문 보기</span></div><div class="session-search-controls"><input id="sessionSearchInput" class="search" type="search" placeholder="대화 내용 검색: 부트스트랩, verify-report, worktree fork"><button id="sessionSearchButton" class="button" type="button">본문 검색</button></div><p id="sessionSearchStatus" class="session-search-status">검색어를 입력하면 JSONL 본문을 스캔합니다. 다음 slice에서 현재 패널/새 패널 이어가기를 붙일 수 있습니다.</p><div id="sessionSearchResults" class="session-search-results"></div></section>`;
}

function renderSessionSearchResults(search: ConversationSearchResponse): string {
	if (!search.terms.length) return `<div class="empty">검색어를 입력하세요.</div>`;
	if (!search.results.length) return `<div class="empty">${escapeHtml(search.query)}와 일치하는 대화 본문을 찾지 못했습니다. 스캔한 세션: ${search.scanned}</div>`;
...
(function(){
	...
	window.showTab = function(tab, opts){
		...
		filterCards();
		if (tab === 'session-search') setTimeout(() => qs('#sessionSearchInput')?.focus(), 0);
	...
	async function requestSessionSearch(){
		const input = qs('#sessionSearchInput');
		const button = qs('#sessionSearchButton');
		const status = qs('#sessionSearchStatus');
		const results = qs('#sessionSearchResults');
		const query = (input?.value || '').trim();
		if (!query) { if (status) status.textContent = '검색어를 입력하세요.'; if (results) results.innerHTML = ''; return; }
		const previous = button?.textContent || '본문 검색';
		if (button) { button.disabled = true; button.textContent = '검색 중...'; }
		if (status) status.textContent = '대화 JSONL 본문을 스캔하는 중...';
		try {
			const response = await fetch('/session-search?q=' + encodeURIComponent(query));
			const payload = await response.json().catch(() => ({}));
			if (!response.ok || !payload.ok) throw new Error(payload.error || 'session search failed');
			if (results) results.innerHTML = payload.html || '';
			if (status) status.textContent = payload.summary || '검색 완료';
		} catch (error) {
			if (status) status.textContent = '대화 내용 검색 실패: ' + String(error && error.message || error);
		} finally {
	...
	document.addEventListener('keydown', (event) => {
		const target = event.target;
		if (target instanceof HTMLElement && target.id === 'sessionSearchInput' && event.key === 'Enter') {
