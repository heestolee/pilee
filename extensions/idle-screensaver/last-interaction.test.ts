...
test("in-memory entries에 timestamp가 없으면 session JSONL을 source-of-truth로 사용한다", () => {
	...
	try {
		...
		assert.equal(resolved, Date.parse("2026-05-19T09:30:00.000Z"));
	} finally {
		rmSync(join(sessionFile, ".."), { recursive: true, force: true });
	}
});

test("timestamped entries가 있으면 session file과 fallback보다 우선한다", () => {
	const sessionFile = writeSession([
	...
	try {
		...
		assert.equal(resolved, Date.parse("2026-05-19T11:00:00.000Z"));
	} finally {
		rmSync(join(sessionFile, ".."), { recursive: true, force: true });
	}
});
