...
export const configPath = join(getAgentDir(), "custom-style.json");
...
export const defaultConfig: CustomStyleConfig = {
	icons: {
		cwd: "󰝰",
		git: "",
		ahead: "↑",
...
export function loadConfig(): CustomStyleConfig {
	try {
		if (!existsSync(configPath)) return defaultConfig;
		const parsed = JSON.parse(readFileSync(configPath, "utf8")) as Partial<CustomStyleConfig>;
		return {
			icons: {
				...defaultConfig.icons,
				...(parsed.icons ?? {}),
