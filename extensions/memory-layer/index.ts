		...
		(tui, theme, _kb, done) => {
			return {
				render: (w: number) => {
					...
					lines.push(`  ${theme.bold("Memories")} (${mems.length})  Scope: ${theme.fg("accent", scopeLabel)} [s]  ${searchMode ? theme.fg("warning", `Search: ${searchQuery}│`) : theme.fg("dim", "/ to search")}`);
					lines.push(theme.fg("accent", "─".repeat(w)));

					if (viewingId) {
						const mem = mems.find((m) => m.id === viewingId);
						if (mem) {
							lines.push(`  ${theme.fg("accent", `#${mem.id}`)} [${mem.scope}] ${theme.fg("muted", mem.topic)}`);
							lines.push(theme.fg("dim", `  Created: ${new Date(mem.createdAt).toLocaleDateString()}`));
							lines.push("");
							for (const line of mem.content.split("\n")) {
								lines.push(`  ${line}`);
							}
							lines.push("");
							lines.push(theme.fg("dim", "  Esc 돌아가기 · d 삭제"));
						}
					} else if (mems.length === 0) {
						lines.push(theme.fg("muted", searchQuery ? `  "${searchQuery}" 검색 결과 없음` : "  메모리 없음. /remember로 추가하세요."));
					} else {
						...
						for (let i = scrollOffset; i < Math.min(mems.length, scrollOffset + visibleHeight); i++) {
							...
							const scope = m.scope === "user" ? theme.fg("muted", "G") : theme.fg("accent", "P");
							const topic = theme.fg("dim", m.topic.padEnd(15));
					...
					lines.push(theme.fg("accent", "─".repeat(w)));
					lines.push(theme.fg("dim", "  ↑↓ 이동 · Enter 상세 · d 삭제 · s scope · / 검색 · q 닫기"));
					return lines;
				},
