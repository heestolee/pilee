...
function extractReviewState(review: unknown): GithubReviewState | null {
	...
	const state = readString(review.state) ?? readString(review.reviewDecision) ?? readNestedString(review, "latestReview", "state");
...
function extractRequestedUser(request: unknown): { key: string; label: string } | null {
	...
	const login = readNestedString(request, "requestedReviewer", "login") ?? readString(request.login) ?? readString(request.name);
...
function extractRequestedTeam(request: unknown): { key: string; label: string } | null {
	...
	return { key: slug, label: org ? `@${org}/${slug}` : `@${slug}` };
}
...
export function parsePullRequestReviewReRequestPlan(stdout: string): PullRequestReviewReRequestPlan | null {
	try {
		...
		for (const request of asArray(parsed.reviewRequests)) {
			...
			if (team) {
				reviewers.set(`team:${team.key}`, { kind: "team", key: team.key, label: team.label, state: null, isRequested: true });
				continue;
			}
			const user = extractRequestedUser(request);
			if (!user) continue;
			reviewers.set(`user:${user.key}`, { kind: "user", key: user.key, label: user.label, state: null, isRequested: true });
		}
...
function buildPullRequestReviewReRequestArgs(
	...
	for (const reviewer of plan.targetUsers) args.push("-f", `reviewers[]=${reviewer}`);
	for (const team of plan.targetTeams) args.push("-f", `team_reviewers[]=${team}`);
	return args;
}
...
export async function fetchPullRequestReviewReRequestPlan(
	...
	const result = await execGh(pi, cwd, ["pr", "view", String(pullRequest.number), "--json", PR_REVIEWERS_VIEW_JSON_FIELDS]);
