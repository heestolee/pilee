...
function registerFrameV2ForkTool(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		description: "Fork a completed /frame-v2 through the original command context, promote frame.json and tasks, activate its exact session in the selected panel, and start implementation.",
		...
		async execute(toolCallId, params: FrameWorktreeForkParams, _signal, _onUpdate, toolCtx: ExtensionContext) {
			...
			if (result.status !== "activated" && result.status !== "switched") return blockedResult(`BLOCKED: Frame v2 worktree session의 선택한 panel activation이 완료되지 않았습니다. 사유: ${result.reason}`, { action: FRAME_V2_FORK_TOOL, result });
