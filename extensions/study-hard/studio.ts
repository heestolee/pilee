...
export interface StudyNoteBlockPresentation {
	container: "details";
	defaultOpen?: boolean;
	summary?: string;
}
...
export interface StudyNoteBlock {
	...
	presentation?: StudyNoteBlockPresentation;
}
...
interface StudyHardHandle {
	...
	metaReviewStartInFlight?: Promise<StudyMetaReviewLink>;
...
function normalizeNoteBlockPresentation(value: unknown): StudyNoteBlockPresentation | undefined {
	if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
	const item = value as Record<string, unknown>;
	if (String(item.container || "").toLowerCase() !== "details") return undefined;
	return {
		container: "details",
	...
	return value
		...
		.map((item, index) => {
			...
			return {
				...
				visual,
				visualRef,
				presentation: normalizeNoteBlockPresentation(item.presentation),
			};
		});
}

function normalizeUnderstandingBlocks(section: Pick<StudyNoteSection, "id" | "kind" | "title">, blocks: StudyNoteBlock[]): StudyNoteBlock[] {
	const isUnderstandingSection = section.kind === "reflection"
		&& (/understanding[\s_-]*(check|review)/i.test(section.id) || /understanding[\s_-]*(check|review)/i.test(section.title) || /이해\s*확인/.test(section.title));
	if (!isUnderstandingSection) return blocks;
	return blocks.map((block) => {
		if (block.type === "callout") {
			const title = block.title?.trim() || "";
			const isQuestion = block.tone === "question" && /[?？]$/.test(title);
			if (!block.presentation && block.body?.trim() && isQuestion) return { ...block, presentation: { container: "details", defaultOpen: false } };
			return block;
		}
		if (block.type !== "visual" || !block.visual || !block.body?.trim()) return block;
		const presentation = normalizeNoteBlockPresentation(block.visual.presentation);
		if (!presentation) return block;
		return {
			id: block.id,
			type: "callout",
			tone: "question",
			title: presentation.summary || block.title || "이해 확인",
			body: block.body,
			presentation: { ...presentation, summary: presentation.summary || block.title },
		};
	});
}
...
function normalizeNoteDocument(value: unknown, fallbackTitle: string): StudyNoteDocument | undefined {
	...
	const sections = item.sections
		...
		.map((section, index) => {
			...
			const title = String(section.title || id);
			return {
				id,
				kind,
				subjectId: typeof section.subjectId === "string" ? section.subjectId : undefined,
				title,
				blocks: normalizeUnderstandingBlocks({ id, kind, title }, normalizeNoteBlocks(section.blocks) || []),
			};
		});
...
interface MergeBoardStateOptions {
...
function exportNoteBlockHtml(block: StudyNoteBlock, state: StudyHardBoardState, diagramAssets: Map<string, StudyDiagramExportAsset>): string {
	...
	if (block.type === "callout") {
		...
		const title = block.presentation?.summary || block.title || "핵심";
		const body = `<p>${escapeExportHtml(block.body || block.text)}</p>`;
		if (block.presentation?.container === "details") return `<details class="answerDisclosure"${block.presentation.defaultOpen ? " open" : ""}><summary><span><span class="calloutIcon" aria-label="${meta.label}">${meta.icon}</span>${escapeExportHtml(title)}</span><small>내 답과 비교하기 · 펼쳐서 확인</small></summary><div class="answerDisclosureBody">${body}</div></details>`;
		return `<aside class="callout ${escapeExportHtml(block.tone || "info")}"><strong><span class="calloutIcon" aria-label="${meta.label}">${meta.icon}</span>${escapeExportHtml(title)}</strong>${body}</aside>`;
	}
...
function exportReadingFlowHtml(steps: MetaReviewExportSnapshot["readingFlow"]): string {
	if (!steps.length) return "";
	const links = steps.map((step, index) => `<li><a class="exportReadingStep" href="#${escapeExportHtml(step.sectionId)}" data-export-reading-link="${escapeExportHtml(step.sectionId)}"><b>${String(index + 1).padStart(2, "0")}</b><span><strong>${escapeExportHtml(step.path)}</strong><small>${escapeExportHtml(step.reason)}</small></span></a></li>`).join("");
	return `<aside class="exportReadingRail" aria-label="읽는 흐름"><div class="exportReadingHeader"><h2>읽는 흐름</h2><span data-export-reading-percent>0% 읽음</span></div><span class="exportReadingProgress"><i data-export-reading-progress></i></span><ol>${links}</ol><p>${steps.length}개 파일 · 번호 순서대로 읽고 각 단계의 이유를 함께 확인합니다.</p></aside>`;
}

function orderExportSectionsByReadingFlow(sections: StudyNoteSection[], steps: MetaReviewExportSnapshot["readingFlow"]): StudyNoteSection[] {
	const sectionById = new Map(sections.map((section) => [section.id, section]));
	const seen = new Set<string>();
	const readingSections = steps.flatMap((step) => {
		const section = sectionById.get(step.sectionId);
		if (!section || seen.has(section.id)) return [];
		seen.add(section.id);
		return [section];
	});
	if (readingSections.length < 2) return sections;
	const firstReadingIndex = sections.findIndex((section) => seen.has(section.id));
	return sections.flatMap((section, index) => {
		if (!seen.has(section.id)) return [section];
		return index === firstReadingIndex ? readingSections : [];
	});
}

interface StudyNoteExportOptions {
	artifactLabel?: string;
	readingFlow?: MetaReviewExportSnapshot["readingFlow"];
}
...
export function buildStudyNoteExportHtml(state: StudyHardBoardState, diagramAssetList: StudyDiagramExportAsset[] = [], workContract?: Pick<ResolvedStudyHardWorkContract, "title" | "hash" | "markdown">, options: StudyNoteExportOptions = {}): string {
	...
	const readingFlow = options.readingFlow || [];
	const sections = orderExportSectionsByReadingFlow(document.sections, readingFlow).map((section) => exportNoteSectionHtml(section, state, diagramAssets)).join("");
	...
	const readingRail = exportReadingFlowHtml(readingFlow);
	const documentContent = readingRail ? `<div class="exportLayout"><div class="exportDocument">${sections}${companion}</div>${readingRail}</div>` : `${sections}${companion}`;
	return `<!doctype html>
...
function commitHandlePatch(handle: StudyHardHandle, patch: Record<string, unknown>): void {
	handle.state = mergeBoardState(handle.state, patch, { allowQuestionExecutionTransitions: true });
	saveState(handle);
	broadcast(handle);
}
	...
	server.on("request", async (req, res) => {
		try {
			...
			if (pathname === "/meta-review/start" && req.method === "POST") {
				...
				const reused = !!handle.metaReviewStartInFlight;
				if (!handle.metaReviewStartInFlight) {
					handle.metaReviewStartInFlight = (async () => {
						...
						if (handle.state.metaReview) return handle.state.metaReview;
						const updated = updateStudyHardStudio(handle.state.runId, {
							activeSurface: "review",
							metaReview: { ...started, linkedAt: Date.now() },
						});
						return updated.state.metaReview!;
					})().finally(() => {
						handle.metaReviewStartInFlight = undefined;
					});
				}
				const metaReview = await handle.metaReviewStartInFlight;
				sendJson(res, 202, { ok: true, metaReview, reused });
				return;
			}
...
function updateStudyHardStudioState(
	...
	handle.state = mergeBoardState(handle.state, patch, { allowQuestionExecutionTransitions });
	saveState(handle);
	broadcast(handle);
	return handle;
}
...
export function updateStudyHardStudio(runId: string | undefined, patch: Record<string, unknown>, expectedRevision?: number): StudyHardHandle {
	return updateStudyHardStudioState(runId, patch, expectedRevision, false);
}
...
function respondStudyHardQuestionWithOwner(
	runId: string | undefined,
	expectedRevision: number,
	questionId: string,
	feedbackValue: string,
	patch: Record<string, unknown>,
	owner: QuestionExecutionMode,
): StudyHardHandle {
	const id = runId || latestRunId;
	if (!id) throw new Error("활성 Study Hard Studio가 없습니다.");
	...
	if (owner === "direct" && (patch.noteDocument !== undefined || patch.attachments !== undefined)) throw new Error("direct 질문은 학습 노트 canonical을 수정할 수 없습니다. worker로 승격하세요.");
	const execution = studyQuestionExecution(target, owner);
	if (execution.mode !== owner) throw new Error(`${execution.mode || "routing"} 질문을 ${owner} 응답으로 완료할 수 없습니다.`);
	...
	const completedExecution = updateQuestionExecutionPhase(execution, "answered");
	...
	const handle = updateStudyHardStudioState(id, { ...patch, questions, currentQuestionId: questionId }, expectedRevision, true);
...
export function respondStudyHardQuestion(
	...
	return respondStudyHardQuestionWithOwner(runId, expectedRevision, questionId, feedbackValue, patch, "direct");
}
...
function applyWorkerNoteWithAttachments(
	...
	try {
		return {
			handle: respondStudyHardQuestionWithOwner(handle.state.runId, handle.state.revision, questionId, artifact.feedback, {
