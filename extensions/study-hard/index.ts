...
8. 질문과 답변은 하나의 composer에서 범위를 나눠 이어집니다.
   ...
   - Studio 오른쪽 Drawer의 learner 질문은 P0가 실제 \`study-hard-worker --main\` subagent로 dispatch합니다. 여러 질문은 표준 #N worker widget에 각각 보이며 비동기로 병렬 실행합니다.
   ...
   - 적용은 base/proposed/current noteDocument의 3-way merge로 엄격하게 처리합니다. disjoint 변경은 보존하고 같은 필드·삭제 대 수정·양립 불가능한 순서 변경은 silent overwrite하지 않습니다. 첫 conflict는 같은 worker run을 최신 state로 한 번 continue하고 두 번째 conflict는 P0 판단으로 남깁니다.
   - worker 전체 noteDocument JSON은 Pi transcript에 노출하지 않고 artifact에 둡니다. 질문·worker #N 완료 요약·최종 답변·노트 반영·충돌은 같은 P0 transcript에 기록하여 다음 Pi context에도 포함합니다. Glimpse는 별도 대화가 아니라 이 canonical 학습 대화의 UI입니다.
