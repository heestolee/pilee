...
8. 질문과 답변은 하나의 composer에서 범위를 나눠 이어집니다.
   ...
   - Studio 오른쪽 Drawer의 learner 질문은 extension coordinator가 표준 \`study-hard-worker --isolated\` subagent dispatcher로 즉시 보냅니다. 필요한 학습 맥락은 task와 최신 board state로 전달하며 P0 전체 transcript를 복제하지 않습니다. P0 LLM turn은 launch나 정상 completion apply의 gate가 아니며, 여러 질문은 표준 #N worker widget에 각각 보이고 비동기로 병렬 실행합니다.
