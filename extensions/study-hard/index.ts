...
8. 질문과 답변은 하나의 composer에서 범위를 나눠 이어집니다.
   ...
   - worker 전체 noteDocument JSON은 Pi transcript에 노출하지 않고 artifact에 둡니다. 질문·worker #N 완료 요약·최종 답변·노트 반영·충돌은 같은 메인 session lineage에 기록하여 다음 P0 context에도 포함합니다. Glimpse는 별도 대화가 아니라 이 canonical 학습 대화의 UI입니다.
