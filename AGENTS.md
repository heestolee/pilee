...
**서사 작성 원칙 (중요):**
- 커밋 로그 나열이 아니라, **대화에서 나온 맥락**을 담을 것
- 왜 이 작업을 시작했는지 (동기)
- 사용자 피드백으로 방향이 어떻게 바뀌었는지 (전환점)
- 시도했다가 안 돼서 다른 방법을 쓴 과정 (시행착오)
- 최종 결정의 근거

**타이밍:**
- 매 세션 마무리 시: 그 세션의 서사를 append (대화 맥락을 이미 알고 있으니 바로 작성)
- 23:59: 당일 세션들 확인하여 놓친 기록 보정 + 사실 관계 확인 + history-backup 스냅샷

```bash
# 서사 내용을 stdin으로 전달
echo '서사...' | node scripts/sync-notion-log.mjs pilee-history "제목"

# 같은 날짜에 추가하면 기존 페이지에 append
echo '추가 서사...' | node scripts/sync-notion-log.mjs pilee-history "제목"

# 23:59 백업
node scripts/sync-notion-log.mjs pilee-history --backup
```
