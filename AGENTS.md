...
pilee-history.md 기록 후 반드시 Notion why 표에도 서사형 로그를 남길 것.

```bash
# 서사 내용을 stdin으로 전달
echo '오늘 한 일 서사...' | node scripts/sync-notion-log.mjs pilee-history "제목"

# 같은 날짜에 추가하면 기존 페이지에 append
echo '추가 서사...' | node scripts/sync-notion-log.mjs pilee-history "제목"

# 다른 날짜 지정
echo '서사...' | node scripts/sync-notion-log.mjs pilee-history "제목" --date 2026-05-01
```

동작 방식:
- 해당 날짜 페이지가 why 표에 없으면 → 새 페이지 생성 + 서사 작성
- 해당 날짜 페이지가 있으면 → 기존 페이지에 서사 append
